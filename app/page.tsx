    ...
    <div className="min-h-screen bg-slate-950 text-slate-200 p-8 font-sans">
      {!session.isLoggedIn ? (
        <div className="max-w-md mx-auto mt-24 bg-slate-900 p-8 rounded-3xl border border-slate-800 shadow-2xl">
          ...
          <form action={connectRepositoryAction} className="space-y-5">
          ...
          {failures.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              {failures.map((fail: any) => (
                <div key={fail.runId} className="bg-slate-900/80 p-8 rounded-3xl border border-slate-800 hover:border-blue-500/30 transition-all">
                  ...
                  <div className="bg-black/50 rounded-2xl p-5 mb-8 font-mono text-xs text-blue-300 border border-white/5">
                    <p className="text-slate-600 mb-3">// AI Suggested Fix</p>
                    {fail.suggestedFix}
                  </div>

                  {/* Uses .bind to pass arguments safely to the server action */}
                  <form action={healFailureAction.bind(null, fail.runId, repoFullName)}>
